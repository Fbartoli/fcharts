...
> ⚠️ **DRAFT — not signed.** 14 criteria require human attestation (see *Attestation* below) before this report is final.
...
## Summary
**28 Supports · 7 Partially Supports · 20 Not Applicable** across 55 WCAG 2.2 A/AA success criteria.
...
| 1.4.1 Use of Color | A | Partially Supports | Legend, table, and readout give color-free series identity, but on the canvas series are distinguished by color only (no per-series dash/marker), so two close lines cannot be told apart by a color-blind user (deferred R5). The non-color identity channels are checked automatically; on-canvas distinguishability is a perceptual attestation. (verified: hybrid) |
| 1.4.2 Audio Control | A | Not Applicable | The component plays no audio. (verified: automated) |
| 1.4.3 Contrast (Minimum) | AA | Partially Supports | Library DOM text passes AA on a light background (ticks 7.56:1, body 10.31:1, readout 16.98:1) and the legend hidden state no longer dims text (R8). Residual gaps are integrator-dependent: author-supplied canvas series colors (no default palette) and the effective host background. (verified: hybrid) |
| 1.4.4 Resize Text | AA | Partially Supports | Container is fluid so full-page browser zoom (the path WCAG accepts) works. Library gap: tick/axis-title/legend/readout font sizes are absolute px, so text-only zoom does not enlarge them (deferred R7); clip/overlap at 200% page-zoom needs human verification. (verified: hybrid) |
...
| 1.4.10 Reflow | AA | Partially Supports | Fluid canvas with no library min-width, reflowing wrap legend, and the 2-D chart-geometry exception cover reflow. Library gap: fixed-px nowrap tick labels can overlap at ~320 CSS px / high zoom (deferred R7); whether they overlap in a given layout needs human verification. (verified: hybrid) |
| 1.4.11 Non-text Contrast | AA | Partially Supports | Focus ring --sl-focus #2563eb = 5.17:1 on white. Gaps: default grid (1.16:1), axis/border (1.41:1), and cursor crosshair (1.96:1) are below 3:1, and data-mark contrast is author-determined (no default palette); highContrast thickens strokes but does not reach 3:1 (deferred R6). (verified: hybrid) |
...
| 2.5.7 Dragging Movements | AA | Partially Supports | The chart's one dragging operation — drag-to-pan — has no single-pointer non-dragging alternative (the keyboard path satisfies 2.1.1 not 2.5.7, and wheel-zoom changes magnification not lateral position). Library-closable but deferred (R4). (verified: hybrid) |
...
| Windows High Contrast Mode (forced-colors) | Partially Supports | The DOM overlay adapts and the focus indicator is preserved (forced-colors:active gives the surface an outline:2px solid Highlight; DOM text/buttons remap to system colors). Inherent gap: the canvas bitmap cannot participate in forced-colors, so series/grid/crosshair stay author-supplied — the DOM data alternatives carry the user through (deferred R12). (verified: hybrid) |
...
| 4.2.3 | Usage without perception of color | Partially Supports | Series identity is available without color via the legend, data table, and readout; on-canvas marks are color-only, so the data table is the non-color alternative. (derived from 1.4.1) |
...
| 4.2.7 | Usage with limited manipulation or strength | Partially Supports | Full keyboard operation (including zoom); pointer pan currently lacks a single-pointer non-dragging alternative (deferred), so this is partially supported for pointer-only users. (derived from 2.1.1, 2.5.1, 2.5.7, 2.5.8) |
...
- **1.4.1 Use of Color** — hybrid
- **1.4.3 Contrast (Minimum)** — hybrid
- **1.4.4 Resize Text** — hybrid
...
- **2.5.7 Dragging Movements** — hybrid
- **4.1.3 Status Messages** — hybrid
- **forced-colors Windows High Contrast Mode (forced-colors)** — hybrid
