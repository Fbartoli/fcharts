...
| 4.2.7 | Usage with limited manipulation or strength | Supports | Full keyboard operation (including zoom); pointer pan currently lacks a single-pointer non-dragging alternative (deferred), so this is partially supported for pointer-only users. (derived from 2.1.1, 2.5.1, 2.5.7, 2.5.8) |
